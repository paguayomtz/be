...
$area = $cn->real_escape_string(isset($_GET['area']) ? $_GET['area'] : '');
$puesto = $cn->real_escape_string(isset($_GET['puesto']) ? $_GET['puesto'] : '');
$nivel = $cn->real_escape_string(isset($_GET['nivel']) ? $_GET['nivel'] : '');
...
<div class="row">
    <div class="col s12">
        <h6>Curriculums (<?php echo $row ?>) <?php //echo $pb 
                                                ?></h6>
            ...
            <tbody id="cuerpo">
                <?php while ($f = $sel->fetch_row()) { ?>
                    <tr id="pag<?php echo $pag ?>" class="nover">
                        <td><?php echo $pag ?></td>
                        <td>
                            <span class="grey darken-1 white-text valign-wrapper btn-fila obs-perfil apuntador" onclick="loadObs(<?= $f[0] ?>)">
                                <?= $f[1] ?>
                            </span>
                        </td>
                        ...
                        <td class="center">
                            <a class="btn-floating" onclick="loadRes(<?php echo $f[0] ?>,'<?php echo $f[1] ?>')"><i class="material-icons green">assignment</i></a>
                        </td>
                        <td>
...
Utils::deleteSession('obs');
require_once '../extend/scripts.php';
require_once 'modal_observaciones.php';
$cn->close();
