...
$sel = $cn->query("SELECT id_perfil, nombre, ide_area, ide_puesto, ide_nivel, cv FROM perfiles 
                        WHERE ide_area IS NULL OR ide_puesto IS NULL OR ide_nivel IS NULL ORDER BY nombre");
            ...
            <tbody id="cuerpo">
                <?php while ($f = $sel->fetch_row()) { ?>
                    <tr id="pag<?php echo $pag ?>" class="">
                        <!--<td class=""><span class="yellow text-white" style="padding: 5px 10px; border-radius:10%;"><?php //echo $pag 
                                                                                                                        ?></span></td>-->
                        <td class="">
